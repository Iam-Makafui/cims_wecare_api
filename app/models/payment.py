...
class Payment:
    ...
    @staticmethod
    def add_single_payment(member_id, amount, payment_type_id, payment_method_id, payment_month, payment_year, payment_status, user_id):
        try:
            connection = db.engine.raw_connection()
            cursor = connection.cursor()
            cursor.execute(
                "INSERT INTO payments (member_id, amount, payment_method_id, payment_status, payment_type_id, payment_month, payment_year, processed_by) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
                (member_id, amount, payment_method_id, payment_status, payment_type_id, payment_month, payment_year, user_id,)
            )
            connection.commit()  # Commit changes to the database
            cursor.close()
            return {'member_id': member_id, 'amount': amount, 'payment_type_id': payment_type_id, 'payment_method_id': payment_method_id, payment_month: payment_month, 'payment_year': payment_year, 'payment_status': payment_status, 'user_id': user_id}
    ...
    def get_all_payments():
        ...
        cursor.execute("""
                SELECT 
                    members_profile_detail.member_image, members_profile_detail.prefix, members_profile_detail.first_name, members_profile_detail.lastname, members_profile_detail.other_names,
                    members.member_identification_id, members.id AS memberId, payment_amounts.amount, payment_amounts.id AS amountId, payment_amounts.currency, payment_methods.id AS paymentMethodId, 
                    payment_methods.method_name, payments.payment_status, payment_types.id AS paymentTypeId, payment_types.payment_type, payments.month, payments.year, profiles.profile_id, profiles.first_name, profiles.last_name,
                    payments.inserted_at  
                FROM 
    ...
    @staticmethod
    def update_payment_detail(member_id, amount_id, payment_method_id, payment_type_id, payment_month, payment_year, id):
        try:
            connection = db.engine.raw_connection()
            cursor = connection.cursor()
            cursor.execute("""
                UPDATE payments 
                SET member_id = %s, amount_id = %s, payment_method_id = %s, payment_type_id = %s, payment_month = %s, payment_year = %s  
                WHERE id = %s
            """, (member_id, amount_id, payment_method_id, payment_type_id, payment_month, payment_year, id,))
            connection.commit()
            cursor.close()
            connection.close()
            return {'id': id, 'member_id': member_id, 'amount_id': amount_id, 'payment_method_id': payment_method_id, 'payment_type_id': payment_type_id, 'payment_month': payment_month, 'payment_year': payment_year}
        except Exception as e:
            print(e)
            return None
